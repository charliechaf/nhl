...
summary(combined)
#to do list
##1. change data type for necessary columns to do regression and machine learning
##2. add other stats

new = combined$Pts, combined$GF, combined$GA, combined$wins,
combined$losses, combined$`5v5 FO%`, combined$`pp FO%`, combined$`sh FO%`,
variable.names(combined)

drop =c("Team","Record","Home", "Away","Div","Cnf","ICF","unique.fivevfive.Team.","city","Team.y","games","pp Games","sh Games", "5v5 Games")
combined = combined[,!(names(combined) %in% drop)]
combined= combined[complete.cases(combined),]

# combined$`pp FO%` = as.numeric(str_sub(combined$`pp FO%`, start = 1, end = 4))/100
# combined$`sh FO%` = as.numeric(str_sub(combined$`sh FO%`, start = 1, end = 4))/100
# combined$`5v5 FO%` = as.numeric(str_sub(combined$`5v5 FO%`, start = 1, end = 4))/100
